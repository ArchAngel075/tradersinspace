...
function Window:new(x,y,w,h,parent)
    ...
    parallel.waitForAny(function() Agent:getShips();  end, spinwait)
    

    for i = 1,#Agent.ships do
        local ship = Agent.ships[i]
        table.insert(self.options,ship:getAttribute('registration').name)
    end
...
function Window:onSubmit()
    ...
    if option == 'back' then
        ...
        local ship = Agent.ships[self.selected-1]
        _w = API.UI.windows.ShipShow(ship,1,1,w,h)
        current_window = _w
    end
end
